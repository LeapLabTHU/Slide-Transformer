...
class SlideAttention(nn.Module):
    def __init__(
        self, input_resolution, dim, num_heads, ka, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., padding_mode='zeros'):
        ...
        self.dep_conv = nn.Conv2d(self.head_dim, self.ka*self.ka*self.head_dim, kernel_size=self.ka, bias=True, groups=self.head_dim, padding=self.ka//2, padding_mode=padding_mode)
        self.dep_conv1 = nn.Conv2d(self.head_dim, self.ka*self.ka*self.head_dim, kernel_size=self.ka, bias=True, groups=self.head_dim, padding=self.ka//2, padding_mode=padding_mode)
    ...
    def forward(self, x, mask=None):
        ...
        k = (self.dep_conv(k) + self.dep_conv1(k)).reshape(B, self.num_heads, self.head_dim, self.ka*self.ka, H, W)
        v = (self.dep_conv(v) + self.dep_conv1(v)).reshape(B, self.num_heads, self.head_dim, self.ka*self.ka, H, W)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if is_slide:
            self.attn = SlideAttention(input_resolution=input_resolution, 
                                       dim=dim, num_heads=num_heads, ka=ka,
...
class SlideSwinTransformer(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 ka=3, num_slide_stages=2, **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               ka=ka,
                               is_slide=(i_layer < num_slide_stages))
